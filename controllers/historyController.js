...
const getHistory = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json({ msg: 'Get history success', history: history });
};
...
const getHistoryByType = async (req,res) => {
    ...
    if (!history) {
        ...
        res.status(StatusCodes.OK).json({ msg: `Get history with type: ${type} success`, history: history });
}
...
const getHistoryByStatus = async(req,res) =>{
    ...
    if (!history) {
        throw new badRequestError(`Can not find any history with status: ${status}`);
        }
    
        res.status(StatusCodes.OK).json({ msg: `Get history with status: ${status} success`, history: history });
...
const getHistoryByFromUser = async(req,res) =>{
    ...
    if (!history) {
        throw new badRequestError(`Can not find any history have transaction from User: ${fromUser}`);
        }
    
        res.status(StatusCodes.OK).json({ msg: `Get history have transaction from User: ${fromUser} success`, history: history });
}
...
const getHistoryByToUser = async(req,res) =>{
    ...
    if (!history) {
        throw new badRequestError(`Can not find any history have transaction to User: ${toUser}`);
        }
...
const getHistoryByUserLogin = async (req, res) => {
    ...
    if (!history) {
        ...
        res.status(StatusCodes.OK).json({ msg: `Get history have transaction user: ${getUser.username} success`, history: history });
}
